<!--- 
	...
	it is also called from SLCMS's OnRequestStart code if the flush serial gets changed so we can update SLCMS code without having to do a full CFWheels reload
 --->

<cflog file="#Application.SLCMS.Logging.theSiteLogName#" type="Information" text="Application Startup: _OnApplicationStart_inc started - Time: #DateFormat(Now(),"YYYYMMDD")#-#TimeFormat(now(),"HH:mm:ss.L")#">
...
<cflog text='Application Startup after base objects loaded in __OnApplicationStart_inc - Time: #DateFormat(Now(),"YYYYMMDD")#-#TimeFormat(now(),"HH:mm:ss.L")#'  file="#Application.SLCMS.Logging.theSiteLogName#" type="Information" application="yes">
...
<!--- set flags then test to make sure we have a viable database --->
...
<!--- if we want to run the Install Wizard from the above test log it otherwise lets load in the SLCMS application and get everything going --->	
<cfif application.SLCMS.config.startup.initialization.WeNeedToCreateDBTables or this.variables.SLCMS.ForceInstallWizard>	
	<!--- log we need the install wizard, it actually runs in OnRequestStart --->
  <cflog text="SLCMS Initialization code skipped in ApplicationStart. WeNeedToCreateDBTables said: #YesNoFormat(application.SLCMS.config.startup.initialization.WeNeedToCreateDBTables)#  - Time: #DateFormat(Now(),"YYYYMMDD")#-#TimeFormat(now(),"HH:mm:ss.L")#" type="Information" file="#Application.SLCMS.Logging.theSiteLogName#">
<cfelse>
  ...
  <cflog text="Application Startup: initialize the core Started - Time: #DateFormat(Now(),"YYYYMMDD")#-#TimeFormat(now(),"HH:mm:ss.L")#" type="Information" file="#Application.SLCMS.Logging.theSiteLogName#">
	...
	<cfif this.variables.SLCMS.CodingMode>
		Core Initialized:-<br>
		UserControl Init dump in __OnAppStart_inc:<br>
		<cfdump var="#this.variables.SLCMS.Initialization.CoreDefinitionInitReturn#" expand="false">
		<br>
	</cfif>
	
  <cflog text="Application Startup: initialize the Modules Started - Time: #DateFormat(Now(),"YYYYMMDD")#-#TimeFormat(now(),"HH:mm:ss.L")#" type="Information" file="#Application.SLCMS.Logging.theSiteLogName#">
	...
	<cfset this.variables.SLCMS.Initialization.ModuleManagerInitReturn = application.SLCMS.System.ModuleManager.init(ApplicationConfig=application.SLCMS.config) />
	
	<cfif this.variables.SLCMS.CodingMode>
		Modules System Initialized:-<br>
		application.SLCMS.System in __OnAppStart_inc:<br>
		<cfdump var="#application.SLCMS.System#" expand="false" label="application.SLCMS.System">
		application.Modules in __OnAppStart_inc:<br>
		<cfdump var="#application.SLCMS.Modules#" expand="false" label="application.SLCMS.Modules">
		<br>
	</cfif>
  <cflog text="Application Startup: initialize the Modules Finished - Time: #DateFormat(Now(),"YYYYMMDD")#-#TimeFormat(now(),"HH:mm:ss.L")#" type="Information" file="#Application.SLCMS.Logging.theSiteLogName#">
...
<cfset structDelete(application.SLCMS.config.StartUp, "FlushAppGlobals") />
...
<cfif this.variables.SLCMS.CodingMode>
	...
	<!---
		application.SLCMS.Core.UserControl.getVariablesScope() at end of _OnAppStart_inc:<br>
...
<cflog text='Application Startup: _OnApplicationStart_inc finished - Time: #DateFormat(Now(),"YYYYMMDD")#-#TimeFormat(now(),"HH:mm:ss.L")#'  file="#this.variables.SLCMS.theSiteLogName#" type="Information" application="yes">
