...
<!--- the ModuleController components are not persistent 
			but we can set up the odd var in the pseudo constructor here to make maintenance easy
			 --->
<cfset variables.Version = "0.0.0.445" /> <!--- the version of this code set --->
<cfset variables.theModuleFriendlyName = "_Empty_Module" /> <!--- formal name of this module, it _has_ to be the folder name tha the module is within --->
<cfset variables.theModuleFriendlyName = "_Empty_Module" /> <!--- friendly name of this module, usually similar to the the folder name but does not have to be. Used in error reporting and menus --->
...
<cffunction name="getBaseDefinition" output="yes" returntype="struct" access="public"
	...
	<cftry>
		<cfif isStruct(arguments.ApplicationPaths) and structkeyexists(arguments.ApplicationPaths, "SitePhysicalRoot")>	<!--- simple check to make sure we have the right data to calculate paths from --->
			...
			<cfset ret.Data.DisplayTypes.TypeList = "Type1,Type2" />	<!--- list of what basic displays we can do. Has to be at least one, if there is a front end --->
			<!--- NOTE! edit these two as required and manually add in the structure of each display type if more than these --->
			<cfset ret.Data.DisplayTypes.Type1 = StructNew() />	<!--- first in list of display types we can do --->
			<cfset ret.Data.DisplayTypes.Type1.Mode = "Template" />	<!--- flag to show which to show by default [Template|Tag] --->
			...
			<cfset ret.Data.DisplayTypes.Type2 = StructNew() />	<!--- second in list of display types we can do --->
			<cfset ret.Data.DisplayTypes.Type2.Mode = "Tag" />	<!--- flag to show which to show by default --->
			...
			<cfset ret.Data.Flags.PortalAware = False />	<!--- does not know about subsites so this module can only be global --->
			...
			<cfset ret.Data.Paths.Templates = "Templates/" />	<!--- the folder that has templates to use in displaying module content --->
			<cfset ret.Data.Paths.TemplateList = "Template1.cfm" />	<!--- list all of the templates that you want to be selected in the page properties drop-down --->
...
<cffunction name="initModule" 
	...
	<!--- first we set OnS variables as the init()s below use them, again this is cloned code from the photogallery --->
...
<cffunction name="ReInitAfter" output="yes" returntype="struct" access="public"
	...
	<cfset loc.theAction = trim(arguments.Action) />
	<!--- now vars that will get filled as we go --->
	<cfset loc.temp1 = False />	<!--- temp/throwaway var --->
	<cfset loc.tempStruct2 = StructNew() />	<!--- temp/throwaway structure --->
	...
	<cftry>
		...
		<cfswitch expression="#loc.theAction#">
