...
<!--- modified: 20th Mar 2012 - 20th Mar 2012 by Kym K, mbcomms: moved search update to "publish" not immediately after normal save --->
<!--- modified: 25th Nov 2012 - 25th Nov 2012 by Kym K, mbcomms: V3+ (CFW version) pulled out wiki code as wiki is now in its own module --->
	...
	<!--- then work out if the content we are showing is straight content, a wiki item or whatever --->
	<cfif thisTag.theDisplayType eq "">
		<!--- if no supplied attribute use whatever the page is (allow for legacy params) --->
		<cfif ListLen(request.SLCMS.PageParams.Param2) eq 1>
			<cfset thisTag.theDisplayType = request.SLCMS.PageParams.Param2 />
		<cfelse>
			<cfset thisTag.theDisplayType = ListLast(request.SLCMS.PageParams.Param2) />
		</cfif>
	</cfif>
	<!--- sort out the display type and what content it is --->
	<!--- 
	<cfif thisTag.theDisplayType eq "Standard">
	 --->
		<cfset theContentTypeID = 1 />
		<cfset thisTag.flagIsInShowContentNormallyMode = True />
		<cfset thisTag.flagContentIsWiki = False />
		<cfset thePageID = request.SLCMS.PageParams.DocID />
<!--- 	
	<cfelseif thisTag.theDisplayType eq "wiki">
		<cfset theContentTypeID = 5 />
		<cfset thisTag.flagIsInShowContentNormallyMode = True />
		<cfset thisTag.flagContentIsWiki = True />
		<cfset thePageID = request.SLCMS.PageParams.wikibits.wikiDocID />

		<!--- check to see if its the wiki home page or a bit of its content --->
		<cfif request.SLCMS.PageParams.wikibits.wikiPageName eq "">
			<!--- it is the home page so the DocID in the PageParams is the ID of the wiki's parent page --->
			<cfset thePageID = request.SLCMS.PageParams.DocID />
		<cfelse>
			<!--- we need to get this wiki page's id --->
			<cfset thePageID = request.SLCMS.PageParams.wikibits.wikiDocID />
		</cfif>
 	<cfelse>
		<cfset theContentTypeID = 1 />
...
--->
 ...
 --->
				<cfset thisTag.VersionOutCountTemp = thisTag.theContentControlData.VersionOutCount+1>
				<cfset thisTag.ContentGetModeMode = "Latest"  />
				...
				<cfset thisTag.theContentControlData.VersionOutCount = thisTag.VersionOutCountTemp />
				<cfset thisTag.flagIsInEditMode = False  />
			<cfelse>
				<!--- oops! or cancelled --->
				<cfset thisTag.flagIsInEditMode = False  />
	...
	<!--- 
	<cfif thisTag.flagContentIsWiki>
		...
		</cfif>	<!--- end: in edit mode --->
	</cfif>
	<cfif thisTag.flagContentIsWiki and thisTag.theContent eq "">
  ...
  <div class="ContentContainer_Wrapper">
			...
			<cfelse>
				...
				<cfif thisTag.IsEditor>	<!--- Editors can publish, the author can't --->
					<cfif thisTag.theContentControlData.VersionOutCount gt 0 and thisTag.theContentControlData.ContentStatus eq "Live">
						This is the published version, there <cfif thisTag.theContentControlData.VersionOutCount neq 1>are<cfelse>is</cfif> #thisTag.theContentControlData.VersionOutCount# newer version<cfif thisTag.theContentControlData.VersionOutCount neq 1>s</cfif>.<br>
					...
					<cfelseif thisTag.theContentControlData.VersionOutCount gt 0 and thisTag.theContentControlData.ContentStatus eq "Latest">
						<cfif thisTag.theContentControlData.VersionOutCount eq 1>
							This is the latest version, the published version is the previous version.
						<cfelse>
							This is the latest version, it is #thisTag.theContentControlData.VersionOutCount# version<cfif thisTag.theContentControlData.VersionOutCount neq 1>s</cfif> newer than the published version.
						</cfif>
						<br>
					...
					<cfelseif thisTag.theContentControlData.VersionOutCount gt 0 and thisTag.theContentControlData.ContentStatus neq "Live">
						There <cfif thisTag.theContentControlData.VersionOutCount neq 1>are<cfelse>is</cfif> #thisTag.theContentControlData.VersionOutCount# version<cfif thisTag.theContentControlData.VersionOutCount neq 1>s</cfif> newer than the published version.<br>
						<input type="image" name="PublishButton" value="Publish_Version" src="#request.SLCMS.rootURL#SLCMS/SLCMSstyling/nbutton_publish.gif" border="0" title="Publish this content">
						or 
						<input type="image" name="ListVersions" value="List Other Versions" src="#request.SLCMS.rootURL#SLCMS/SLCMSstyling/nbutton_ListOtherVersions.gif" border="0" title="List Other Versions of this content">
						or 
					...
					<cfelseif thisTag.theContentControlData.VersionOutCount lt 0 and thisTag.theContentControlData.ContentStatus neq "Live">
						This version is #abs(thisTag.theContentControlData.VersionOutCount)# version<cfif thisTag.theContentControlData.VersionOutCount neq -1>s</cfif> older than the published version.<br>
						<input type="image" name="PublishButton" value="Publish_Version" src="#request.SLCMS.rootURL#SLCMS/SLCMSstyling/nbutton_publish.gif" border="0" title="Publish this content">
						or 
						<input type="image" name="ListVersions" value="List Other Versions" src="#request.SLCMS.rootURL#SLCMS/SLCMSstyling/nbutton_ListOtherVersions.gif" border="0" title="List Other Versions of this content">
						or 
					...
					<cfelseif thisTag.theContentControlData.VersionOutCount eq "" and not thisTag.theContentControlData.LiveVersion>
						No content has yet been published, there are other versions, this is <cfif thisTag.theContentControlData.Version eq 0>the latest version<cfelse>version #thisTag.theContentControlData.Version#</cfif>. <br>
						<input type="image" name="PublishButton" value="Publish_Version" src="#request.SLCMS.rootURL#SLCMS/SLCMSstyling/nbutton_publish.gif" border="0" title="Publish this content">
						or 
						<input type="image" name="ListVersions" value="List Other Versions" src="#request.SLCMS.rootURL#SLCMS/SLCMSstyling/nbutton_ListOtherVersions.gif" border="0" title="List Other Versions of this content">
					<cfelseif thisTag.theContentControlData.VersionOutCount eq 0 and thisTag.theContentControlData.LiveVersion>
						This content is the published version.&nbsp;
						<input type="image" name="ListVersions" value="List Other Versions" src="#request.SLCMS.rootURL#SLCMS/SLCMSstyling/nbutton_ListOtherVersions.gif" border="0" title="List Other Versions of this content">
	...
	<cfif thisTag.theContentControlData.ContentID eq -1 or not thisTag.flagIsInEditMode>
		...
		<cfelse>
			...
			<!--- but if its a wiki we need to wrap it and show the discussion tabs, etc --->
			<!--- 
			<cfif thisTag.theDisplayType eq "wiki">
