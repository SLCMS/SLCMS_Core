...
<!--- modified: 30th Aug 2009 -  1st Sep 2009 by Kym K - mbcomms: modifying code for session roles, ie the permissions engine --->
...
<cfif thisTag.executionMode IS "start">
	...
	<cfif application.core.UserPermissions.IsLoggedin() eq False and session.user.security.LoginAttempted>
		<cfset tempdata.theUserName = session.user.security.AttemptedUserName />
		<cfset tempdata.thePassword = session.user.security.AttemptedPassword />
	<cfelse>
	...
	<cfif tempdata.theFormAction eq "" and ListFirst(request.PageParams.PageQueryString, "=") eq "from">
		<cfset tempdata.theFromPlace =  ListLast(request.PageParams.PageQueryString, "=")>
		<cfif application.Core.PageStructure.getDocIDfromURL(URLpath="#tempdata.theFromPlace#", SubSiteID="#request.PageParams.SubSiteID#").DocID neq 0>
	...
	<cfoutput>
<!---		
		<cfdump var="#application.Core.UserControl.getVariablesScope()#" expand="false" label="application.Core.UserControl.getVariablesScope">
		<cfdump var="#session.user#" expand="false" label="session">
	--->	
	...
	<cfif application.core.UserPermissions.IsLoggedin() eq False>
		<cfif session.user.security.LoginAttempted>
