<cfcomponent output="No"
	displayname="Empty Core CFC" 
	hint="typical CFC used as a skeleton to build a working CFC, can also be used to build a module CFC using same base components"
	>
<!--- 
...
<cffunction name="init" output="No" returntype="any"	access="public" 
	...
	<cfset temps = LogIt(LogType="CFC_Init", LogString="_Empty Core Init() Started") />
	<cftry>
		<!--- do init stuff here --->
...
<cffunction name="ReInitAfter" output="No" returntype="any" access="public" 
	...
	<cfset ret.error = StructNew() />
	<cfset ret.error.ErrorCode = 0 />
	<cfset ret.error.ErrorContext = "System_ModuleManager CFC: ReInitAfter()" />
