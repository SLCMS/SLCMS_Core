...
<!--- modified:  7th Jun 2011 -  8th Jun 2011 by Kym K, mbcomms: added logging functions so we can have consistent logging outside CF's logs --->
	...
	<cfset variables.CoreFunctionsReInitGeneratorList = "" />	<!--- this is a list of the functions in the Core that can affect modules. They will fill it as their Definitions are read --->
	<cfset variables.DataBaseTableName_ModuleManagement_Base = "" />
	<cfset variables.DataBaseTableName_ModuleManagement_Permissions = "" />
	<cfset variables.ModuleList_Available = "" />	<!--- modules which can be used --->
	<cfset variables.ModuleList_Bad = "" />	<!--- will carry a list of any Modules that did not load in properly and must be ignored --->
	<cfset variables.ModuleList_InFolder = "" />	<!--- all the modules in the folder, good, bad or ugly --->
	<cfset variables.ModulesBaseFyzPath = "" />
	<cfset variables.ModulesBaseURLPath = "" />
	<cfset variables.Modules = StructNew() />	<!--- we are going to keep management data on the modules here, not the full works --->
	<cfset variables.ModuleSort = ArrayNew(1) />	<!--- we are going to keep the init() order of the modules here. Lowest number is last to be init --->
...
<cffunction name="init" output="No" returntype="any"	access="public" 
	...
	<cftry>
		...
		<cflog text='Modules Load Started in Init() - Time: #DateFormat(Now(),"YYYYMMDD")#-#TimeFormat(now(),"HH:mm:ss.L")#'  file="#Application.Logging.theSiteLogName#" type="Information" application="yes">
		<cfset variables.ModuleList_Available = "" />
		<cfset variables.ModuleList_Bad = "" />
		<cfset variables.ModuleList_InFolder = "" />
		<cfset variables.ModulesBaseFyzPath = arguments.applicationConfig.StartUp.SiteBasePath & arguments.applicationConfig.base.SLCMSModulesBaseRelPath />
		<cfset variables.ModulesBaseURLPath = arguments.applicationconfig.base.MapURL & arguments.applicationconfig.base.SLCMSModulesBaseRelPath />
		<cfset variables.DataBaseTableName_ModuleManagement_Base = arguments.applicationConfig.DatabaseDetails.DataBaseTableNaming_Root_System & arguments.applicationConfig.DatabaseDetails.ModuleBaseTable />
		<cfset variables.DataBaseTableName_ModuleManagement_Permissions = arguments.applicationConfig.DatabaseDetails.DataBaseTableNaming_Root_System & arguments.applicationConfig.DatabaseDetails.ModulePermissionsTable />
		...
		<cfelse>
			...
			<cfset variables.ModuleSort[thisLevel] = StructNew() />
			<cfloop list="#variables.ModuleList_Available#" index="thisModule">
				<cfset variables.ModuleSort[thisLevel]["#thisModule#"] = StructNew() />
				<cfset variables.ModuleSort[thisLevel]["#thisModule#"].ReInitAfterList_Core = variables.Modules["#thisModule#"].Dependencies.Core.ReInitFromCoreCFCList />
				<cfset variables.ModuleSort[thisLevel]["#thisModule#"].ReInitAfterList_Module = variables.Modules["#thisModule#"].Dependencies.Module.ReInitFromModulesList />
				<!---
				<cfif variables.Modules["#thisModule#"].Dependencies.Module.ReInitFromModulesList neq "">
					<cfset variables.ModuleSort[thisLevel]["#thisModule#"].ReInitAfterList = ListAppend(variables.ModuleSort[thisLevel]["#thisModule#"].ReInitAfterList, variables.Modules["#thisModule#"].Dependencies.Module.ReInitFromModulesList) />
				</cfif>
				--->
			</cfloop>
			...
			<cfloop condition="oneMovedUp eq True">
				...
				<cfloop collection="#variables.ModuleSort[thisLevel]#" item="thisModule">
					<!--- grab the list of priors for this module --->
					<!--- note we are only fussing about the relative order between modules, not the core --->
					<cfset thisModuleInitList = variables.ModuleSort[thisLevel]["#thisModule#"].ReInitAfterList_Module />
					...
					<cfloop list="#thisModuleInitList#" index="thisModuleInner">
						<cfif not ListFindNoCase(beenMovedUpList, thisModuleInner)>
							...
							<cfif topLevel lte thisLevel>
								...
								<cfset variables.ModuleSort[topLevel] = StructNew() />
							</cfif>
							<!--- add to the new level --->
							<cfset variables.ModuleSort[topLevel]["#thisModuleInner#"] = StructNew() />
							<cfset variables.ModuleSort[topLevel]["#thisModuleInner#"].ReInitAfterList_Core = variables.Modules["#thisModuleInner#"].Dependencies.Core.ReInitFromCoreCFCList />
							<cfset variables.ModuleSort[topLevel]["#thisModuleInner#"].ReInitAfterList_Module = variables.Modules["#thisModuleInner#"].Dependencies.Module.ReInitFromModulesList />
							<!---
							<cfif variables.Modules["#thisModuleInner#"].Dependencies.Module.ReInitFromModulesList neq "">
								<cfset variables.ModuleSort[topLevel]["#thisModuleInner#"].ReInitAfterList = ListAppend(variables.ModuleSort[topLevel]["#thisModuleInner#"].ReInitAfterList, variables.Modules["#thisModuleInner#"].Dependencies.Module.ReInitFromModulesList) />
							</cfif>
							--->
							<!--- and take away from this one --->
							<cfset temp = StructDelete(variables.ModuleSort[thisLevel], thisModuleInner) />
			...
			we do this from the top down in the way we have built this stack
			 --->
			<cfloop from="#ArrayLen(variables.ModuleSort)#" to="1" index="thisLevel" step="-1">
				<cfloop collection="#variables.ModuleSort[thisLevel]#" item="thisModule">
					<cflog text='Module Init() Called - Module #thisModule# - Time: #DateFormat(Now(),"YYYYMMDD")#-#TimeFormat(now(),"HH:mm:ss.L")#'  file="#Application.Logging.theSiteLogName#" type="Information" application="yes">
					<cfset ModuleInitGet = StructNew() />	<!--- just in case clear out junk from previous loops --->
					<cfinvoke component="#application.Modules['#thisModule#'].Paths.ModuleRootURLpath#ModuleController" method="initModule" returnvariable="ModuleInitGet">
						<cfinvokeargument name="ApplicationConfig" value="#application.SLCMS.config#">
						<cfinvokeargument name="ModuleConfig" value="#application.Modules['#thisModule#']#">
					</cfinvoke>
				</cfloop>
			</cfloop>
		</cfif>
		...
		<cflog text='Modules Load Finished in Init() - Time: #DateFormat(Now(),"YYYYMMDD")#-#TimeFormat(now(),"HH:mm:ss.L")#'  file="#Application.Logging.theSiteLogName#" type="Information" application="yes">
	<cfcatch type="any">
	...
	<cfset temps = LogIt(LogType="CFC_Init", LogString="System_ModuleManager Finished") />
...
<cffunction name="ReadModulesFolder" output="No" returntype="any" access="public" 
	...
	<cftry>
		...
		<cfset variables.ModuleList_InFolder = "" />
		<cfset variables.ModuleList_Bad = "" />
		<cfdirectory action="list" directory="#Variables.ModulesBaseFyzPath#" name="Initialization.ModuleDirectoryQuery">
		<cfloop query="Initialization.ModuleDirectoryQuery">
			...
			<cfif Initialization.ModuleDirectoryQuery.Type eq "dir" and Initialization.thisModuleFilename neq ".svn" and Not (FindNoCase(" ", Initialization.thisModuleFilename) or FindNoCase("H", Initialization.ModuleDirectoryQuery.attributes))>
				...
				<cfset Initialization.ModuleFyzPath = "#Variables.ModulesBaseFyzPath##Initialization.thisModuleFilename#/" />
				<cfset Initialization.ModuleURLPath = "#Variables.ModulesBaseURLPath##Initialization.thisModuleFilename#/" />
				...
				<cfif FileExists(Initialization.ModuleDefFyzPath)>
					...
					<cfset variables.ModuleList_InFolder = ListAppend(variables.ModuleList_InFolder, Initialization.thisModuleFilename) />
					...
					<cfelse>
						...
						<cflog text='ReadModuleFromFolder(#Initialization.thisModuleFilename#) failed. Error was: #Initialization.ret1.error.ErrorText# - Time: #DateFormat(Now(),"YYYYMMDD")#-#TimeFormat(now(),"HH:mm:ss")#'  file="#Application.Logging.theSiteLogName#" type="Error" application = "yes">
				...
				<cfelse>
					...
					<cfset ret.error.ErrorCode =  BitOr(ret.error.ErrorCode, 1) />
					<cflog text='ReadModuleFromFolder(#Initialization.thisModuleFilename#) failed. No ModuleController.cfc Found - Time: #DateFormat(Now(),"YYYYMMDD")#-#TimeFormat(now(),"HH:mm:ss")#'  file="#Application.Logging.theSiteLogName#" type="Error" application = "yes">
		...
		<cfelse>
			...
			<cfset temp = variables.ModuleList_Available />	<!--- grab the list of hopefully good modules and put in temp var as we are going to mangle the original list as we go if we find bad ones --->
			<cfloop list="#temp#" index="thisAvailableModule">
				<!--- grab the modules that each module needs to be able to run and check it exists --->
				<cfset theNeeds =  variables.Modules["#thisAvailableModule#"].Dependencies.Module.NeededModulesList />
				<cfloop list="#theNeeds#" index="thisNeed">
					<cfif not StructKeyExists(application.Modules, "#thisNeed#")>
						<!--- poo it is not there --->
						<cfset variables.ModuleList_Bad = ListAppend(variables.ModuleList_Bad, thisAvailableModule) />
						<cfif ListFind(ModuleList_Available, thisAvailableModule)>
							<cfset variables.ModuleList_Available = ListDeleteAt(variables.ModuleList_Available, ListFind(ModuleList_Available, thisAvailableModule)) />
						</cfif>
						<cflog text='Missing Module Need. Module #thisAvailableModule# needs module #thisNeed# to run and its not there. - Time: #DateFormat(Now(),"YYYYMMDD")#-#TimeFormat(now(),"HH:mm:ss")#'  file="#Application.Logging.theSiteLogName#" type="Error" application = "yes">
		...
		<cfset application.system.ModuleList_Available = variables.ModuleList_Available />
		<cfset application.system.ModuleList_Bad = variables.ModuleList_Bad />
		<cfset application.system.ModuleList_InFolder = variables.ModuleList_InFolder />
		<cfcatch type="any">
			<cfset ret.error.ErrorCode =  BitOr(ret.error.ErrorCode, 128) />
...
<cffunction name="ReadModuleFromFolder" output="No" returntype="any" access="public" 
	...
	<cftry>
		<cfif theModulename neq "">
			...
			<cfset Initialization.ModuleFyzPath = "#Variables.ModulesBaseFyzPath##theModulename#/" />
			<cfset Initialization.ModuleURLPath = "#Variables.ModulesBaseURLPath##theModulename#/" />
			<cfset Initialization.ModuleDefFyzPath = "#Initialization.ModuleFyzPath#ModuleController.cfc" />
			<cfset Initialization.ModuleDefURLPath = "#Initialization.ModuleURLPath#ModuleController" />
			...
			<cfinvoke component="#Initialization.ModuleDefURLPath#" method="getBaseDefinition" returnvariable="Initialization.ModuleDefGet">
				<cfinvokeargument name="ApplicationPaths" value="#application.Paths_Common#">
    	</cfinvoke>
			...
			<cfif StructKeyExists(Initialization.ModuleDefGet, "error") and StructKeyExists(Initialization.ModuleDefGet.error, "errorcode") and Initialization.ModuleDefGet.error.errorcode eq 0>
				...
				<cfif StructKeyExists(Initialization.ModuleDefGet, "data") 
					...
					<cfset Variables.ModuleList_Available = ListAppend(Variables.ModuleList_Available, theModulename) />	<!--- its good so add it to the list of workable modules --->
					<cfset Variables.Modules["#theModulename#"] = StructNew() />
					...
					<cfset Variables.Modules["#theModulename#"] = duplicate(Initialization.ModuleDefGet.data) />	
					...
					<cfloop list="#Variables.Modules["#theModulename#"].Dependencies.Core.ReInitFromCoreCFCList#" index="thisCFC">
						<cfif not ListFindNoCase(variables.CoreFunctionsReInitGeneratorList, thisCFC)>
							<cfset variables.CoreFunctionsReInitGeneratorList = ListAppend(variables.CoreFunctionsReInitGeneratorList, thisCFC) />
						</cfif>
					</cfloop>
					<!--- shuffle the supplied data from the module into the App scope --->
					<cfset application.Modules["#theModulename#"] = StructNew() />
					<cfset application.Modules["#theModulename#"].FormalName = theModulename />
					<cfset application.Modules["#theModulename#"].FriendlyName = Variables.Modules["#theModulename#"].ModuleNaming.FriendlyName />
					<cfset application.Modules["#theModulename#"].Description = Variables.Modules["#theModulename#"].ModuleNaming.Description />
					<cfset application.Modules["#theModulename#"].DisplayTypes = duplicate(Initialization.ModuleDefGet.data.DisplayTypes) />
					<cfset application.Modules["#theModulename#"].ModuleAdmin = duplicate(Initialization.ModuleDefGet.data.ModuleAdmin) />
					<cfset application.Modules["#theModulename#"].Paths = duplicate(Initialization.ModuleDefGet.data.Paths) />
					<cfset application.Modules["#theModulename#"].Paths.ModuleRootPhysicalPath = Initialization.ModuleFyzPath />
					<cfset application.Modules["#theModulename#"].Paths.ModuleRootURLPath = Initialization.ModuleURLPath />
					<cfset application.Modules["#theModulename#"].Paths.ApplicationModulesStructureName = "application.Modules" />
					<cfset application.Modules["#theModulename#"].Paths.ApplicationModulesThisStructureName = "#theModulename#" />
					<!--- and add to that with a few peers (spelling? peer as in take a look, peek as in stickybeak) into folders --->
					<cfset application.Modules["#theModulename#"].Paths.PresentationURLRoot = application.Modules["#theModulename#"].Paths.ModuleRootURLPath & application.Modules["#theModulename#"].Paths.Templates />
					<cfset Initialization.TemplateFyzPath = "#Initialization.ModuleFyzPath##application.Modules["#theModulename#"].Paths.Templates#" />
					<cfset Initialization.TemplateTagsFyzPath = "#Initialization.ModuleFyzPath##application.Modules["#theModulename#"].Paths.TemplateTags#" />
					<cfset application.Modules["#theModulename#"].Templates = "" />
					...
					<cfloop query="Initialization.TemplateDirectoryQuery">
						<cfif Initialization.TemplateDirectoryQuery.Name neq ".svn" and Not (FindNoCase(" ", Initialization.TemplateDirectoryQuery.Name) or FindNoCase("H", Initialization.TemplateDirectoryQuery.attributes))>
							<cfset application.Modules["#theModulename#"].Templates = ListAppend(application.Modules["#theModulename#"].Templates, Initialization.TemplateDirectoryQuery.Name) />
						</cfif>
					</cfloop>
					<cfset application.Modules["#theModulename#"].TemplateTags = "" />
					...
					<cfloop query="Initialization.TemplateTagsDirectoryQuery">
						<cfif Initialization.TemplateTagsDirectoryQuery.Name neq ".svn" and Not (FindNoCase(" ", Initialization.TemplateTagsDirectoryQuery.Name) or FindNoCase("H", Initialization.TemplateTagsDirectoryQuery.attributes))>
							<cfset application.Modules["#theModulename#"].TemplateTags = ListAppend(application.Modules["#theModulename#"].TemplateTags, Initialization.TemplateTagsDirectoryQuery.Name) />
						</cfif>
					</cfloop>
				<cfelse>
					<!--- oops! the module definition returned a bad structure --->
					<!--- ToDo: error handler here --->
					<cfset variables.ModuleList_Bad = ListAppend(variables.ModuleList_Bad, theModulename) />
					<cfset ret.error.ErrorCode =  BitOr(ret.error.ErrorCode, 2) />
					<cfset ret.error.ErrorText = ret.error.ErrorText & ' Bad data structure returned from Module: #theModuleName#' />
					<cflog text='Bad data structure returned from Module: #theModulename# - Time: #DateFormat(Now(),"YYYYMMDD")#-#TimeFormat(now(),"HH:mm:ss")#'  file="#Application.Logging.theSiteLogName#" type="Error" application = "yes">
			...
			<cfelse>
				...
				<cfset ret.error.ErrorCode =  BitOr(ret.error.ErrorCode, 1) />
				<cflog text='Bad error structure returned from Module: #theModuleName# - Time: #DateFormat(Now(),"YYYYMMDD")#-#TimeFormat(now(),"HH:mm:ss")#'  file="#Application.Logging.theSiteLogName#" type="Error" application = "yes">
...
<cffunction name="CheckNsetModule2DB" output="No" returntype="any" access="public" 
	...
	<cftry>
		<cfif theModulename neq "">
			...
			<cfset getModuleFromDB = application.core.DataMgr.getRecords(tablename="#variables.DataBaseTableName_ModuleManagement_Base#", data=theQueryWhereArguments, fieldList="ModuleID,ModuleFormalName,Enabled_Global,Enabled_subSiteList,Removed") />
			<cfif getModuleFromDB.RecordCount eq 0>
				<!--- its not in the database so it must be a new module, lets add it in --->
				<!--- first locally --->
				<cfset Variables.Modules["#theModulename#"].Flags.Enabled_Global = False />
				<cfset Variables.Modules["#theModulename#"].Flags.Enabled_subSiteList = "" />
				<!--- then the database --->
				...
				<cfset InsertModule = application.core.DataMgr.InsertRecord(tablename="#variables.DataBaseTableName_ModuleManagement_Base#", data=theQueryDataArguments) />
			<cfelseif getModuleFromDB.RecordCount eq 1>
				<!--- found it so set up the flags for what it can do, etc --->
				<cfset Variables.Modules["#theModulename#"].Flags.Enabled_Global = getModuleFromDB.Enabled_Global />
				<cfset Variables.Modules["#theModulename#"].Flags.Enabled_subSiteList = getModuleFromDB.Enabled_subSiteList />
			<cfelse>
...
<cffunction name="ChangeModuleEnableState" output="No" returntype="any" access="public" 
	...
	<cfset ret.error = StructNew() />
	<cfset ret.error.ErrorCode = 0 />
	<cfset ret.error.ErrorContext = "System_ModuleManager CFC: EnableModule()" />
	<cfset ret.error.ErrorText = "" />
	<cfset ret.error.ErrorExtra = "" />
	<cfset ret.Data = "" />	<!--- and no data yet --->

	<cftry>
		<cfif theModulename neq "" and listFindNoCase(variables.ModuleList_Available, theModulename) and theChange eq "Enable" or theChange eq "Disable">
			<cfif theChange eq "Enable" or theChange eq "Disable">
				<cfif thesubSite eq "Global">
					<!--- lets compare with the database and set things up as we need --->
					<!--- first locally --->
					<cfif theChange eq "Enable">
						<cfset Variables.Modules["#theModulename#"].Flags.Enabled_Global = True />
					...
					<cfset theQueryWhereArguments.ModuleFormalName = theModulename />
					<cfset theQueryDataArguments.Removed = False />
					<cfset setModule = application.core.DataMgr.UpdateRecords(tablename="#variables.DataBaseTableName_ModuleManagement_Base#", data_set=theQueryDataArguments, data_where=theQueryWhereArguments) />
				<cfelseif ListFind(application.core.PortalControl.GetFullSubSiteIDList(), thesubSite)>
					<cfif theChange eq "Enable">
						<cfset temp1 = variables.Modules["#theModulename#"].Flags.Enabled_subSiteList />
						<cfset variables.Modules["#theModulename#"].Flags.Enabled_subSiteList = ListAppend(temp1, thesubSite) />
					<cfelse>
						<cfset temp1 = ListFind(Variables.Modules["#theModulename#"].Flags.Enabled_subSiteList, thesubSite) />
						<cfset Variables.Modules["#theModulename#"].Flags.Enabled_subSiteList = ListDeleteAt(Variables.Modules["#theModulename#"].Flags.Enabled_subSiteList, temp1) />
					</cfif>
					<cfset theQueryWhereArguments.ModuleFormalName = theModulename />
					<cfset theQueryDataArguments.Enabled_subSiteList = variables.Modules["#theModulename#"].Flags.Enabled_subSiteList />
					<cfset setModule = application.core.DataMgr.UpdateRecords(tablename="#variables.DataBaseTableName_ModuleManagement_Base#", data_set=theQueryDataArguments, data_where=theQueryWhereArguments) />
				<cfelse>	
...
<cffunction name="getAvailableModulesFlags" output="No" returntype="struct" access="public"
	...
	<cfset ret.Data.ModuleList = variables.ModuleList_Available />	<!---  what modules we can use --->
	...
	<cfloop list="#variables.ModuleList_Available#" index="thisModule">
		...
		<cfset ret.Data["#thisModule#"].FriendlyName = variables.Modules["#thisModule#"].ModuleNaming.FriendlyName />
		<cfset ret.Data["#thisModule#"].Flags = variables.Modules["#thisModule#"].Flags />
	</cfloop>
	<cfcatch type="any">
		<cfset ret.error.ErrorCode =  BitOr(ret.error.ErrorCode, 128) />
		<cfset ret.error.ErrorText = ret.error.ErrorContext & ' Trapped. Site: #application.SLCMS.config.base.SiteName#, error message was: #cfcatch.message#, error detail was: #cfcatch.detail#' />
...
<cffunction name="SystemHasModules" output="No" returntype="boolean" access="public"
	...
	>
	<cfif variables.ModuleList_Available neq "">
		<cfreturn True />
	<cfelse>
		<cfreturn False />
	</cfif>
...
<cffunction name="getQuickAvailableModulesList" output="No" returntype="string" access="public"
	...
	hint="returns the variables.ModuleList_Available, no error handling"
				>
	<cfreturn variables.ModuleList_Available  />
</cffunction>
...
<cffunction name="ReInitModulesAfter" output="No" returntype="struct" access="public"
	...
	<cfset ret.Data = "" />	<!--- and no data yet --->
		...
		<!--- validation --->
	<cfif theInitiatingModule neq "" and ((theInitiatingModule eq "Core" and ListFindNoCase(variables.CoreFunctionsReInitGeneratorList , theInitiatingFunction)) or ListFindNoCase(variables.ModuleList_Available , theInitiatingModule))>
		<cfif theAction neq "">
			<!--- validated so go for it --->
			<cftry>
			<cfset temps = LogIt(LogType="CFC_Init", LogString="System_ModuleManager ReInitModulesAfter() Started") />
			<!---
			<cflog text='ReInitModulesAfter Started - Time: #DateFormat(Now(),"YYYYMMDD")#-#TimeFormat(now(),"HH:mm:ss.L")#'  file="#Application.Logging.theSiteLogName#" type="Information" application="yes">
			--->
			<!--- 
			...
			we do this from the top down in the way we have built this stack so they reinit in the correct sequence
			 --->
			<cfloop from="#ArrayLen(variables.ModuleSort)#" to="1" index="thisLevel" step="-1">	<!--- step down through array --->
				<cfloop collection="#variables.ModuleSort[thisLevel]#" item="thisModule">	<!--- loop over modules at this level --->
					<cfif theInitiatingModule eq "Core">
						<cfset thisReInitList = variables.ModuleSort[thisLevel]["#thisModule#"].ReInitAfterList_Core />
					<cfelse>
						<cfset thisReInitList = variables.ModuleSort[thisLevel]["#thisModule#"].ReInitAfterList_Module />
					</cfif>
					<cfif ListFindNoCase(thisReInitList, theInitiatingFunction)>
						<!--- this module is dependent on this action so reinit it --->
						<cfset ModuleInitGet = StructNew() />	<!--- just in case clear out junk from previous loops --->
						<cftry>
							<cfinvoke component="#application.Modules['#thisModule#'].Paths.ModuleRootURLpath#ModuleController" method="ReInitAfter" returnvariable="ModuleInitGet">
			...
			<cflog text='ReInitModulesAfter Finished - Time: #DateFormat(Now(),"YYYYMMDD")#-#TimeFormat(now(),"HH:mm:ss.L")#'  file="#Application.Logging.theSiteLogName#" type="Information" application="yes">
			<cfcatch type="any">
				<cfset ret.error.ErrorCode =  BitOr(ret.error.ErrorCode, 128) />
				<cfset ret.error.ErrorText = ret.error.ErrorContext & ' Trapped. Site: #application.SLCMS.config.base.SiteName#, error message was: #cfcatch.message#, error detail was: #cfcatch.detail#' />
				<cfset ret.error.ErrorExtra =  cfcatch.TagContext />
	...
	<cfset temps = LogIt(LogType="CFC_Init", LogString="System_ModuleManager ReInitModulesAfter() Finished") />
	<!--- return our data structure --->
	<cfreturn ret  />
</cffunction>
...
<cffunction name="callModuleCoreAPIcfc" output="No" returntype="struct" access="public"
	...
	<cftry>
		...
		<cfif theModule neq "">
			<cfif theMethod neq "">
				<cfif ListFindNoCase(variables.ModuleList_Available, theModule)>
					<!--- semi validated so go for it --->
					<cfif StructKeyExists(variables.Modules["#theModule#"].CoreAdminAPI, "CoreAPIcfcName")>
						<cfset theCoreAPIcfcName = variables.Modules["#theModule#"].CoreAdminAPI.CoreAPIcfcName />
						<cfset theComponent = variables.modules['#theModule#'].Paths.CFCroot & theCoreAPIcfcName />
...
<cffunction name="callQuickModuleCoreAPIcfc" output="No" returntype="string" access="public"
			...
			<cfif theMethod neq "">
				<cfif ListFindNoCase(variables.ModuleList_Available, theModule)>
					...
					<cfif StructKeyExists(variables.Modules["#theModule#"].CoreAdminAPI, "CoreAPIcfcName")>
						...
						<cfcatch type="any">
							<cflog text='Method call failed. Method called was: #theMethod#; CFC called was: #theComponent# - Time: #DateFormat(Now(),"YYYYMMDD")#-#TimeFormat(now(),"HH:mm:ss")#'  file="#Application.Logging.theSiteLogName#" type="Error" application = "yes">
							<cfif application.SLCMS.config.debug.debugmode>
	...
	<cfcatch type="any">
		<cflog text='System_ModuleManager CFC: callQuickModuleCoreAPIcfc()  Trapped. Site: #application.SLCMS.config.base.SiteName#, error message was: #cfcatch.message#, error detail was: #cfcatch.detail# - Time: #DateFormat(Now(),"YYYYMMDD")#-#TimeFormat(now(),"HH:mm:ss")#'  file="#Application.Logging.theSiteLogName#" type="Error" application = "yes">
		<cfif application.SLCMS.config.debug.debugmode>
...
<cffunction name="getQuickPreProcessorTagList" output="No" returntype="string" access="public"
	...
	<cftry>
		...
		<cfif theModule neq "">
				<cfif ListFindNoCase(variables.ModuleList_Available, theModule)>
					<!--- semi validated so go for it --->
					<cfset ret = variables.Modules["#theModule#"].CoreAdminAPI.PreProcessor.TagList />
				</cfif>
		</cfif>	<!--- end: check for legit module --->
	<cfcatch type="any">
...
<cffunction name="getModuleContentTypeSelector" output="No" returntype="struct" access="public"
	...
	<cfset ret.Data.OptionArray = [] />	<!--- empty display array for selectors --->
		...
		<!--- validation --->
	<cfif theModule neq "" and ListFindNoCase(variables.ModuleList_Available , theModule)>
		<cfif theContentType neq "">
			<cfif ListFindNoCase(variables.ModuleList_Available, theModule)>
				<!--- semi validated so go for it --->
				<cftry>
					<cfif StructKeyExists(variables.Modules["#theModule#"].CoreAdminAPI.PageProperties.ContentSelectors, "#theContentType#")>
						<cfset strContentType = variables.Modules["#theModule#"].CoreAdminAPI.PageProperties.ContentSelectors["#theContentType#"] />
						...
						<cfif strContentType.ConnectorType eq "Page">
							...
							<cfset ret.Data.PopURL = Application.Modules["#theModule#"].ModuleAdmin.AdminRootURL_Abs />
						...
						<cfelseif strContentType.ConnectorType eq "CFC">
							...
							<cfinvoke component="#variables.modules['#theModule#'].Paths.CFCroot#.#theCFCname#" method="#theMethodName#" returnvariable="retAPIcall">
								<cfinvokeargument name="subSiteID" value="#thesubSiteID#">
								<cfinvokeargument name="UserID" value="#theUserID#">
							</cfinvoke>
							<cfif retAPIcall.error.errorcode eq 0>
