...
<cfcomponent output="yes"
	displayname="content.cfm Functions" 
	hint="contains standard utilities to work within content.cfm"
	>
...
<cffunction name="getDocFromURL" output="yes" returntype="struct" access="public"
	...
	<cfelseif IsDefined("cgi.Script_Name")>
		...
		<!--- strip any params that might be tagged on the end, can be in URL but should not be here and why take chances --->
		<cfset ret.Data.theParams = ListFirst(ret.Data.theParams, "?") />
		...
		<cfif len(ret.Data.theParams)>
			...
			<cfset ret.Data.SubSiteID = ret.Data.DocIDStruct.SubSiteID />	<!--- if we have wound down from the portal top to a subsite this could have changed --->
