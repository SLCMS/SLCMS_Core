...
--->
<!--- modified: 16th Nov 2009 - 16th Nov 2009 by Kym K - mbcomms: modified for SLCMS V2.2+
																							functions changed:	cffmDirectoryList - added ignore svn folders/files
																																	 --->
...
<cffunction access="public" name="cffmDirectoryList" output="true" returnType="query" HINT="List the contents of a directory, optionally recursing into subdirectories.  This was implemented due to inconsistencies between Coldfusion and Bluedragon.">
	<cfargument name="directory" type="string" required="true">
	<cfargument name="recurse" type="boolean" required="false" default="false">
	<!--- temp vars --->
	<!--- example:  list = cffmDirectoryList(dir,"","","true"); --->
	...
	<!--- 
		...
		<cfreturn temp>
		 --->
		<!--- this is a reversion to the previous version's code to allow for the filtering out of svn folders --->
		<cfset temp1 = queryNew("attributes,datelastmodified,mode,name,size,type,directory,fullPath")>
		<cfif server.os.name contains "Windows">
			<cfset path = "\">
		<cfelse>
			<cfset path = "/">
		</cfif>
		<cfdirectory name="temp2" directory="#arguments.directory#">
		<cfloop query="temp2">
			<cfif not (temp2.type eq "dir" and temp2.name eq ".svn")>
				<cfset queryAddRow(temp1)>
				<cfset querySetCell(temp1,"attributes",temp2.attributes)>
				<cfset querySetCell(temp1,"datelastmodified",temp2.datelastmodified)>
				<cfset querySetCell(temp1,"mode",temp2.mode)>
				<cfset querySetCell(temp1,"name",temp2.name)>
				<cfset querySetCell(temp1,"size",temp2.size)>
				<cfset querySetCell(temp1,"type",temp2.type)>
				<cfset querySetCell(temp1,"directory",temp2.directory)>
				<Cfset querySetCell(temp1,"fullPath",temp2.directory & path & temp2.name)>
			</cfif>
		</cfloop>
		<cfreturn temp1>
	<cfelse>
		...
		<cfset thisDir = cffmDirectoryList(directory,false)>
		<cfif server.os.name contains "Windows">
			<cfset path = "\">
		<cfelse>
			<cfset path = "/">
		...
		<cfloop query="thisDir">
			<cfif not (thisDir.type eq "dir" and thisDir.name eq ".svn")>
			...
			</cfif>	<!--- end: ignore svn stuff test --->
		</cfloop>
		<cfreturn dirInfo>
	</cfif>
</cffunction>
