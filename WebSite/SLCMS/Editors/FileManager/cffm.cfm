...
--->
<!--- 
 modified: 14th Nov 2009 - 16th Nov 2009 by Kym K - mbcomms: modified for SLCMS V2.2+
																										code changes: are marked by 	/* **SLCMS change** */ or 	<!---  **SLCMS change**  ---> so we can update for the next version
																										code changed:	added __SetParams include at top - line 19
																																	changed init function for include paths to use above - lines 76,77
																																	added our own struct for saving modified paths and things, set in __SetParams
																																	path calculations for formatted display at lines 368 and 393
																																	path formatter code at line 367
																																	not show svn folder in directory list - lines 960-963
																																	fudged data display at line 1152 as date format does not work on our file datestamp format
																																	 --->																																	

<!--- **SLCMS change** added: set up the params we need to initialize --->
<cfinclude template="__SetParams.cfm" />
...
<cfinvoke component="#cffm#" method="init">
	...
	<cfinvokeargument name="includeDirWeb" value="./custom">
	 --->
	<!--- **SLCMS change** our version --->
	<cfinvokeargument name="includeDir" value="#theIncludeBaseDir#">
	<cfinvokeargument name="includeDirWeb" value="#theIncludeBaseURL#">

	<!--- disallowedExtensions = file extensions you don't want people to upload --->
	<cfinvokeargument name="disallowedExtensions" value="cfm,cfml,cfc,dbm,dbml,php,php3,php4,php5,asp,aspx,pl,plx,pls,cgi,jsp,pif,scr,vbs,exe,bat,com">
	<!---
	// allowedExtensions = 
	...
	<cfinvokeargument name="editableExtensions" value="txt,html,htm,log,csv,js,css">
	<!---
	// overwriteDefault = 
...
<!---  **SLCMS change** two extra functions for the page display --->
<cffunction name="loopNavPage" 
	access="private" output="No" returntype="any" 
	...
	<cfloop from="1" to="#ArrayLen(ArrayPart)#" index="lcntr">
		...
		<cfset session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].dispRowCounter = session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].dispRowCounter+1 />
		<cfset session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].displayedRowArray[session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].dispRowCounter][2] = arguments.ArrayPart[lcntr].DocID />
		<cfif session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].dispRowCounter gt 1>
			<!--- if we are not at the first row grab the one above and load that in --->
			<cfset session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].displayedRowArray[session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].dispRowCounter][1] = session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].displayedRowArray[session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].dispRowCounter-1][2] />
		<cfelse>
			<!--- if we at the top --->
			<cfset session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].displayedRowArray[session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].dispRowCounter][1] = 0 />
		</cfif>
		<!--- set up a blank one below until we get there in the next loop --->
		<cfset session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].displayedRowArray[session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].dispRowCounter][3] = 0 />
		<cfif session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].dispRowCounter gte 2>
			<!--- if we are up to the second row load this row into the row above as the below item --->
			<cfset session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].displayedRowArray[session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].dispRowCounter-1][3] = session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].displayedRowArray[session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].dispRowCounter][2] />
		</cfif>
		...
		<cfif arguments.ArrayPart[lcntr].IsParent neq 0 and session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].ExpansionFlags[arguments.ArrayPart[lcntr].DocID] eq True>
...
<cffunction name="dispNavPage" 
	...
	<cfif ArrayItem.IsParent eq true and session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].ExpansionFlags[ArrayItem.DocID] eq True>
		<cfset ShowExpandControl = True />
		<cfset theExpandImage = "Minus16.gif" />
		<cfset theExpandControl = "CollapseArm" />
	<cfelseif ArrayItem.IsParent eq true and session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].ExpansionFlags[ArrayItem.DocID] eq False>
		<cfset ShowExpandControl = True />
	...
	<!--- we only have style for 4 levels so do a force for really deep stuff --->
	...
	<cfif ShowExpandControl>
		<cfset theSubContent = repeatString('<img src="graphics/Blank16.gif" height="16" width="16" border="0">', ArrayItem.ThisLevel-1) />
		<cfset theSubContent = theSubContent & '<a href="#cgi.Script_Name#?mode=#theExpandControl#&amp;DocID=#ArrayItem.DocID#" class="WorkTableNavExpand"><img src="graphics/#theExpandImage#" width="16" height="16" border="0"></a>' />
	<cfelse>
		<cfset theSubContent = '<span class="WorkTableNavExpand"><img src="graphics/Blank16.gif" height="16" width="16" border="0"></span>' & repeatString('<img src="graphics/Blank16.gif" height="16" width="16" border="0">', ArrayItem.ThisLevel-1) />
	</cfif>
	<cfif ArrayItem.HasContent neq 0>
		<cfset theSubContent = theSubContent & '<img src="graphics/document16.gif" height="16" width="16" border="0">' />
	<cfelse>
		<cfset theSubContent = theSubContent & '<img src="graphics/Blank16.gif" height="16" width="16" border="0">' />
	</cfif>
	<cfif ArrayItem.IsParent neq 0>
		<cfset theSubContent = theSubContent & '<img src="graphics/folder16.gif" height="16" width="16" border="0" hspace="2">' />
	</cfif>
	...
	<!--- oo, this is messy, too many single and double quotes to do it in one go --->
	<cfset theHTML = theHTML & '#theSubContent# #ArrayItem.NavName#' />
	<cfif ArrayItem.IsHomePage>
		<cfset theHTML = theHTML & ' <strong>*</strong>' />
	</cfif>
	<cfset theHTML = theHTML & '</span></td>' />
	<!--- npow to work out the stuff to put in the js callback, basically the path to the page --->
	<!--- as we are doing this on the fly rather than neatly as in the page Structure admin we must work our the parentage of each page as we go --->
	<cfset theURLPathToHere = ArrayItem.URLNameEncoded />	<!--- throw in the page name and then add the parents to the front --->
	<cfset theParentID = ArrayItem.ParentID />	<!--- preset the rentrantness of it all --->
	<cfif ArrayItem.ParentID neq 0>
		<cfloop condition="#theParentID# neq 0">
			<cfset theParentPage = application.core.pageStructure.getSingleDocStructure(DocID=theParentID, SubSiteID=session.WYSIWYGEditor.CurrentSubSiteID) />
			<cfset theParentID = theParentPage.ParentID />	<!--- now we loop up one --->
			<cfset theURLPathToHere = theParentPage.URLNameEncoded &"/"& theURLPathToHere />
		</cfloop>
	</cfif>
	<cfif left(theURLPathToHere,1) neq "/">
		<cfset theURLPathToHere = "/"& theURLPathToHere />
	</cfif>
	<cfif len(theURLPathToHere) gt 1 and right(theURLPathToHere,1) eq "/">
		<cfset theURLPathToHere = removechars(theURLPathToHere,len(theURLPathToHere)-1,1)  />
	</cfif>
	...
	<cfset theHTML = theHTML & '<a href="javascript:OpenFile(' />
	<cfset theHTML = theHTML & "'#application.config.base.rootURL#content.cfm#theURLPathToHere#'" />
	<cfset theHTML = theHTML & ');" target="_self"><span class="#theNavNameClass#">Select</span>' />
	<cfset theHTML = theHTML & '</td>' />
	<cfset theHTML = theHTML & '</tr>' />
...
<cfscript>
	...
	*/
	...
	/* **SLCMS addition** */
	variables.slscmsMods.FullPathToWorkingFolder = variables.workingDirectoryWeb;
</cfscript>
<cfscript>
...
<cfif variables.action eq "download">
	...
	<cfabort>
<cfelseif variables.action eq "get">
	<cfcontent type="#cffm.getMimeType(downloadFilename)#" file="#variables.workingDirectory##dirsep##downloadFilename#">
	<cfabort>
</cfif>
...
<cfscript>
	...
	if (variables.action eq "delete") {
		...
		if (variables.uploadResults.errorCode neq 0)
		{
			...
			fckUploadResult(variables.uploadResults.errorCode,'','',cffm.resourceKit.errorMsg.t25);
		} else {
		...
		if (variables.editFilename contains "/" or variables.editFilename contains "\") {
			...
			if (NOT imageLoaded)
			{
				...
				try {
					...
					// cffmdump(e);
					variables.errorMessage = variables.errorMessage & "<li>#e.message# - #e.detail#.</li>#Chr(10)#";
					imageWritten = false;
				}
				if (not imageWritten)
				{
...
<!---  **SLCMS change** added more friendly paths  --->
<cfset variables.slscmsMods.LengthOfPrePart = len("#application.config.base.RootURL#Sites/#variables.slscmsMods.SubSiteShortName#/Resources/") />	<!--- our working folder taken from variables.workingDirectoryWeb --->
<cfset variables.slscmsMods.ShortPathToWorkingFolder = RemoveChars(variables.slscmsMods.FullPathToWorkingFolder,1,variables.slscmsMods.LengthOfPrePart) />	<!--- will have short version without "/sites/subsite_nn/Resources" at front --->
...
<cfif cffm.templateWrapperAbove neq "">
	...
	<cfsetting enablecfoutputonly="yes">
<cfelse>
	<cfoutput><HTML><HEAD><TITLE>SLCMS File Manager</TITLE></HEAD><BODY></cfoutput>
	<cfhtmlhead text="<link rel=stylesheet type=text/css href=#application.config.base.rootURL#Global/SLCMS_FileManagerStyles.css>">
</cfif>
...
<cfif session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].ResourceType eq "File">
	<p class="cffm_FilePageChooser">
	<cfif session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].FileMode eq "ShowFiles">
		Current Display is File Resources <a href="#cgi.script_name#?FileMode=ShowPages">Change to <strong>Display Pages</strong> in site</a>
	<cfelse>
		Current Display is Site Pages <a href="#cgi.script_name#?FileMode=ShowFiles">Change to <strong>Display File Resources</strong> in site</a>
	</cfif>
	</p>
</cfif>
<cfif session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].FileMode eq "ShowFiles">
<!---  **SLCMS change** just show the short path below the subsite ---> 
	<p class="cffm_location">#cffm.resourcekit['Msg.t1']#:  #variables.slscmsMods.ShortPathToWorkingFolder#</p>
</cfif>
</cfoutput>
<!---  **SLCMS change** add the Filemode switch ---> 
<cfif listFind("edit,viewsource,viewzip,renameForm,copymoveForm,manipulateForm",action) gt 0 and FileMode eq "ShowFiles">
...
<cfscript>
	...
	<p><img src="#variables.linkToFile#" borer=5 width="#variables.displayWidth#" height="#variables.displayHeight#"></p>
	</cfoutput>
...
<cfelseif action eq "list" and FileMode eq "ShowFiles">
<!---  **SLCMS change**  ---> 
	<!--- replaced by kym 20091116 with the function directoryList() so it is svn aware
	<cfdirectory action="LIST" directory="#variables.workingDirectory#" name="variables.dirList">
	 --->
	<cfset variables.dirList = cffm.cffmDirectoryList(variables.workingDirectory) />
...
<!--- 
<cfdump var="#variables.dirList#" expand="false">
 --->
...
<input type="text" size="30" name="createNewFilename" onFocus="select();"><br>&nbsp;<br>&nbsp;
...
if (type eq "dir") {
	...
	<!--- this is a fudge for the date format until we sort out what the standards formats are across all OSs --->
	<cfset theModifiedDateLastModified = Createdate("20#Left(ListLast(dateLastModified,"/"),2)#",ListGetAt(dateLastModified,2,"/"),ListFirst(dateLastModified,"/"))/>
	</cfsilent>
	<tr>
...
<cfelseif action eq "list" and FileMode eq "ShowPages">
	...
	<p>&nbsp;</p>
	<table border="1" cellpadding="3" cellspacing="0" class="worktable">
	<tr>
		<td colspan="1" rowspan="1" class="WorkTableTopRow" align="center"><u>Page Name</u><br><span style="font-size:0.8em;">* is Home Page</span></td>
		<td colspan="1" rowspan="1" class="WorkTableTopRow"><u>Menu Level</u></td>
		<td colspan="1" rowspan="1" class="WorkTableTopRowRHCol">Select</td>
	</tr>
		<!--- Show all the rows in the display table, this is rentrant --->
		<cfoutput>#loopNavPage(session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].theCurrentNavArray)#</cfoutput>
	</table>	<!--- end: worktable --->
<!--- 
<cfdump var="#session.WYSIWYGEditor["#session.WYSIWYGEditor.CurrentContentHandle#"].theCurrentNavArray#" expand="false">
 --->
